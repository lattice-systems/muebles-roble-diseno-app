...
class ProductionOrderMaterial(db.Model):
    ...
    production_order = db.relationship(
        "ProductionOrder", back_populates="material_consumptions"
    )

    raw_material = db.relationship(
        "RawMaterial", back_populates="production_order_materials"
    )
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "quantity_planned": (
                float(self.quantity_planned)
                if self.quantity_planned is not None
                else None
            ),
            "quantity_used": (
                float(self.quantity_used) if self.quantity_used is not None else None
            ),
            "unit_cost": float(self.unit_cost) if self.unit_cost is not None else None,
            "waste_applied": (
                float(self.waste_applied) if self.waste_applied is not None else None
            ),
